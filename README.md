...
- `src/assets/cell-reference.svg` is the native SVG used in A. `src/assets/trace_reference_cell.py` regenerates its color paths from a separately supplied reference screenshot, removing baked-in receptors so React can paint them independently. The original screenshot is not bundled in the repository. `src/assets/trace_reference_markers.py` regenerates the optional color-path fragments in `src/components/markers/referenceTraces.tsx` from that same screenshot. These are source-derived vector traces, not embedded runtime rasters. The earlier `cell-a.svg` and the B illustration are retained as separate assets.
