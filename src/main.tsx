...
import { sampleMarkers, markerStudyMarkers } from './markers';
import './styles.css';

function App() {
  const [mode, setMode] = useState<CellMode>('a');
  const [showMarkers, setShowMarkers] = useState(true);
  const [markerSet, setMarkerSet] = useState<'visit' | 'locations'>('visit');
  return <main className="app-shell"><header className="topbar"><div className="brand-icon" aria-hidden="true">✳</div><span>VISIT EXPLAINED <span className="brand-divider">/</span> CELL LAB</span></header>
    ...
    <CellVisualization key={markerSet} mode={mode} markers={markerSet === 'visit' ? sampleMarkers : markerStudyMarkers} showMarkers={showMarkers}/>
    <div className="marker-set" role="group" aria-label="Marker examples"><button type="button" aria-pressed={markerSet === 'visit'} onClick={() => setMarkerSet('visit')}>Visit markers</button><button type="button" aria-pressed={markerSet === 'locations'} onClick={() => setMarkerSet('locations')}>Marker study</button></div>
